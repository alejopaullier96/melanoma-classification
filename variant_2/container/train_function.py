    ...
    for fold, (train_index, valid_index) in enumerate(folds):
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                # Compute PR-ROC
                precision, recall, _ = precision_recall_curve(valid_data['target'].values,
                                                                        valid_preds.cpu())
                ...
                # Append to .txt file
                with open(f"logs/logs_{version}.txt", 'a+') as f:
                    print('{} | Epoch: {}/{} | Loss: {:.4} | Train Acc: {:.3} | Valid Acc: {:.3} | PR-AUC: {:.3}'. \
                          format(duration, epoch + 1, epochs, train_losses, train_acc, valid_acc, pr_auc), file=f)
                # Print to console
                print('{} | Epoch: {}/{} | Loss: {:.4} | Train Acc: {:.3} | Valid Acc: {:.3} | PR-AUC: {:.3}'. \
                      format(duration, epoch + 1, epochs, train_losses, train_acc, valid_acc, pr_auc))
                ...
                # Update best_auc
                if not best_auc:  # If best_auc = None
                    best_auc = pr_auc
                    torch.save(model.state_dict(),
                               f"saved_models/Fold{fold + 1}_Epoch{epoch + 1}_ValidAcc_{valid_acc:.3f}_PR-AUC_{pr_auc:.3f}.pth")
                    continue
                ...
                if pr_auc > best_auc:
                    ...
                    if patience_f == 0:
                        with open(f"logs/logs_{version}.txt", 'a+') as f:
                            print('Early stopping (no improvement since 3 models) | Best PR-AUC: {}'. \
                                  format(best_auc), file=f)
                        print('Early stopping (no improvement since 3 models) | Best PR-AUC: {}'. \
                              format(best_auc))
                        break

        # === INFERENCE ===
        # Choose model with best_auc in this fold
        best_model_path = 'saved_models/' + [file for file in os.listdir('saved_models') if
                                             str(round(best_auc, 3)) in file and 'Fold' + str(fold + 1) in file][0]
